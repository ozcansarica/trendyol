// Ekran görüntüsündeki referans senaryoya göre hesaplama testleri.
...
test('referans senaryo (satis 189, alis 30, kom %22.5, kdv %20, kargo 42)', () => {
  ...
  assert.equal(round2(r.hizmetKdv), 2.2);
  assert.equal(round2(r.kar), 49.5);
});

test('hizmet bedeli aralıkları', () => {
  assert.equal(hizmetBedeli(100), 8.49);
  assert.equal(hizmetBedeli(189), 13.19);
  assert.equal(hizmetBedeli(400), 20.39);
  assert.equal(hizmetBedeli(1000), 34.79);
});
